...
export default function RightPanel({ canvas }: { canvas: fabric.Canvas | null }) {
  const store = useAppStore();
  const [activeObj, setActiveObj] = useState<fabric.Object | null>(null);
  const [bgColor, setBgColor] = useState<string>('#ffffff');

  // Trigger re-render of panel when properties change
  ...
  const updateProp = (key: string, value: any) => {
    if (activeObj && canvas) {
      ...
      setTick(t => t + 1); // force update panel UI
    }
  };
      ...
      <div className="w-72 bg-white border-l h-full p-4 overflow-y-auto">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="bg-gray-50 px-3 py-2 text-sm text-gray-500 rounded border cursor-not-allowed">
              595 × 842 (A4)
            </div>
          </div>
          <div>
  ...
  return (
    <div className="w-72 bg-white border-l h-full p-4 overflow-y-auto hidden-scrollbar">
          ...
          <>
            <div>
              ...
              <div className="mb-2">
                ...
                <div className="flex gap-2 items-center">
                  <input type="color" value={(activeObj.fill as string) || '#000000'} onChange={(e) => updateProp('fill', e.target.value)} disabled={activeObj.fill === 'transparent'} className="w-8 h-8 rounded shrink-0 p-0 border-0" />
